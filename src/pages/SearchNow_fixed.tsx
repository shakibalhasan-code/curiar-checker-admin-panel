        ...
        <div className="p-4 lg:p-6">
            ...
            {phoneData && !isLoading && showResults && (
                <div className="space-y-4 lg:space-y-6">
                    ...
                    {/* Fraud Score & Risk Assessment Combined */}
                    {(() => {
                            ...
                            <div className={`${riskAssessment.bgColor} rounded-lg p-4 lg:p-6`}>
                                <div className="flex items-start space-x-4">
                                    <div className="flex-shrink-0">
                                        <div className={`text-3xl font-bold ${riskAssessment.textColor}`}>
                                            {phoneData.fraudScore}
                                        </div>
                                        <div className="text-sm text-center text-white/70">Score</div>
                                    </div>
                                    <div className="flex-1">
                                        <div className="flex items-center space-x-2 mb-2">
                                            {riskAssessment.icon}
                                            <h3 className={`font-semibold text-lg ${riskAssessment.textColor}`}>
                                                Risk Assessment - {riskAssessment.level.toUpperCase()}
                                            </h3>
                                        </div>
                                        <p className="text-sm text-white/90">
                                            {phoneData.aiAnalysis?.analysis || phoneData.analysis || 'No analysis available'}
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    {/* Courier Data Table - Always show */}
                    <div className="bg-slate-800 rounded-xl p-4 lg:p-6 border border-slate-700">
                        <h3 className="text-white font-semibold mb-4 flex items-center space-x-2">
                            <span>📊</span>
                            <span>Courier Service Performance</span>
                        </h3>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-slate-600">
                                        <th className="text-left text-slate-300 font-medium pb-3 pr-4">Courier</th>
                                        <th className="text-left text-slate-300 font-medium pb-3 pr-4">Status</th>
                                        <th className="text-left text-slate-300 font-medium pb-3 pr-4">Success</th>
                                        <th className="text-left text-slate-300 font-medium pb-3 pr-4">Cancel</th>
                                        <th className="text-left text-slate-300 font-medium pb-3 pr-4">Total</th>
                                        <th className="text-left text-slate-300 font-medium pb-3">Phone</th>
                                    </tr>
                                </thead>
                                <tbody className="text-white">
                                        ...
                                        <tr className="border-b border-slate-700/50 hover:bg-slate-700/30">
                                            ...
                                            <td className="py-3 pr-4">
                                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                                    phoneData.courier_checks.pathao.fraud !== null && phoneData.courier_checks.pathao.fraud !== undefined
                                                        ? 'bg-red-100 text-red-800'
                                                        : 'bg-green-100 text-green-800'
                                                }`}>
                                                    {phoneData.courier_checks.pathao.fraud !== null && phoneData.courier_checks.pathao.fraud !== undefined ? 'Fraud Report' : 'Clean'}
                                                </span>
                                            </td>
                                    ...
                                    {phoneData.courier_checks?.steadfast && (
                                        <tr className="border-b border-slate-700/50 hover:bg-slate-700/30">
                                            ...
                                            <td className="py-3 pr-4">
                                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                                    phoneData.courier_checks.steadfast.fraud !== null && phoneData.courier_checks.steadfast.fraud !== undefined
                                                        ? 'bg-red-100 text-red-800'
                                                        : 'bg-green-100 text-green-800'
                                                }`}>
                                                    {phoneData.courier_checks.steadfast.fraud !== null && phoneData.courier_checks.steadfast.fraud !== undefined ? 'Fraud Report' : 'Clean'}
                                                </span>
                                            </td>
                                            <td className="py-3 pr-4 text-emerald-400 font-medium">
                                    ...
                                    {phoneData.courier_checks?.redx && (
                                        <tr className="border-b border-slate-700/50 hover:bg-slate-700/30">
                                            ...
                                            <td className="py-3 pr-4">
                                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                                    phoneData.courier_checks.redx.fraud !== null && phoneData.courier_checks.redx.fraud !== undefined
                                                        ? 'bg-red-100 text-red-800'
                                                        : 'bg-green-100 text-green-800'
                                                }`}>
                                                    {phoneData.courier_checks.redx.fraud !== null && phoneData.courier_checks.redx.fraud !== undefined ? 'Fraud Report' : 'Clean'}
                                                </span>
                                            </td>
                                            <td className="py-3 pr-4 text-emerald-400 font-medium">
                                    ...
                                    {/* Show message if no courier data */}
                                    {!phoneData.courier_checks?.pathao && !phoneData.courier_checks?.steadfast && !phoneData.courier_checks?.redx && (
                                        <tr>
                                            <td colSpan={6} className="py-6 text-center text-slate-400">
                                                No courier data available for this phone number
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
                    ...
                    {/* Cache Status */}
                    {phoneData.cached && (
                        <div className="bg-blue-900/20 border-blue-500/30 rounded-lg p-3 border">
                            <div className="flex items-center space-x-2">
                                <span className="text-blue-400">ℹ️</span>
                                <span className="text-blue-300 text-sm">This data was retrieved from cache for faster response</span>
                            </div>
                        </div>
                    )}
                </div>
